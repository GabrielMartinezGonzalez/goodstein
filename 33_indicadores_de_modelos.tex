...
En el siguiente capítulo desarrollaremos la teoria de conjuntos $\alpha-$grandes; una poderosa teoría que será exageradamente útil en lo que resta del presente trabajo.
...
\begin{defn} \textit{(conjuntos $\alpha-$grandes)}\\
    ...
    \begin{itemize}
        \item $X$ es \textit{$1-$grande} si $2\leq |X|$;
        \item $X$ es \textit{$\alpha-$grande} si $X\setminus\set{x_0}$ es $F(\al,x_1)-$grande.
    \end{itemize}
\end{defn}
\begin{obs}
    Por como se definió el concepto de $\alpha-$\textit{grande} y la función $F$, tenemos:
    \begin{itemize}
...
\end{lem}
\begin{proof}
    Sin pérdida de generalidad, suponemos $Y$ no vacío. Procedemos por inducción sobre $\alpha$:
    \begin{itemize}
