...
\begin{obs}
    Toda máquina de Turing especifica de manera única una función cuyo dominio y rango son subconjuntos del conjunto de sucesiones finitas de $L$.
\end{obs}
...
\begin{ex}
    ...
    \begin{itemize}
        ...
        \item para cada $n$,
            \[A_n:=\{f | f\in C_n\},\]
            donde $C_n$ es la colección de funciones computables por una máquina de Turing con a lo más $n$ estados (más, quizá, el estado final).
...
\begin{defn} \textit{(función total)}\\
    Una función computable se dice total si para cualquier número natural como input, la función se detiene, es decir, el cálculo de su output se puede obtener en un número finito de pasos. En otras palabras, la función es total si su dominio es $\mathbb{N}^k$.
\end{defn}
...
\end{thm}
\begin{proof}
    Supongamos que este conjunto sí es computable, entonces hay una función computable
    \[l\mapsto f_l,\]
...
\begin{defn} \textit{(conjunto computable)}\\
    Sea $X\subseteq\n^k$. Decimos que $X$ es computable si $\chi_X:\n^k\longrightarrow\{0,1\}$ es una función total computable. Es decir, $X$ es computable si existe un algoritmo con input $n$, tal que determina si $n\in X$ o no.
\end{defn}