...
\begin{proof}
    ...
    \begin{itemize}
        \item Si $\beta=0$ entonces lo de la derecha de {\eqref{1}} es $\alpha\cup\nt=\al$. Lo de la izquierda es $\alpha+0$, lo cual es igual a $\alpha$ por definición.
        \item Asumiendo la ecuación {\eqref{1}} válida para $\gamma$ y $\beta=S(\gamma)$. Entonces, 
            ...
            \[=\alpha+\beta.\]
        \item Ahora, asumimos que $\beta$ es un ordinal límite y que {\eqref{1}} es válida para cada $\gamma<\beta$. Entonces, para cada $\delta<\beta$ existe una $\gamma$ tal que $\delta<\gamma<\beta$, y también:
...
\end{proof}
\begin{cor}\label{c24}
    Supóngase que $\alpha,\beta,\gamma,\delta$ son ordinales tales que $\gamma<\beta$. Entonces,
...
\end{defn}
\begin{thm}\label{t26}
    Sean $\lar{X,\preceq_X}$ y $\lar{Y,\preceq_Y}$ buenos órdenes, y sea $\alpha=\ot(\lar{X,\preceq_X})$, $\beta=\ot(\lar{Y,\preceq_Y})$. Entonces, $\alpha+\beta=\ot(\lar{X,\preceq_X}\oplus\lar{Y,\preceq_Y})$.
\end{thm}
\begin{proof}
...
\end{prop}
\begin{proof}
 Se sigue del teorema {\ref{t26}} y las propiedades de la suma de órdenes.
\end{proof}
\begin{defn}\label{d28}
    ...
    \item $\alpha\cdot\beta=\bigcup_{\gamma\in\beta}(\alpha\cdot\gamma)$, si $\beta$ es un ordinal límite no cero.
    \end{itemize}
\end{defn}
Observemos que 
...
\begin{cor}\label{c31}
    ...
    Supóngase $\alpha,\beta\in\ON$ tales que $1\leq\alpha<\beta$. Entonces, existe un único par de ordinales $\lar{\xi,\eta}$ tales que
...
\begin{proof}
    ...
    \begin{enumerate}[label=\roman*]
        ...
        \item Supongamos $\xi\neq\xi'$. Sin pérdida de generalidad, $\xi<\xi'$. Entonces, $\xi+1\leq\xi'$, y $\alpha\cdot\xi+\eta<\alpha\xi+\alpha=\alpha\cdot(\xi+1)\leq\alpha\xi'\leq\alpha\cdot\xi'+\eta'$, lo cuál contradice {\eqref{4}}.
    \end{enumerate}
\end{proof}
\begin{thm}\label{t32} 
    ...
    con $\otimes^a$ el producto antilexicográfico.{\cite{SetTheory}}
\end{thm}
\begin{cor}
...
\begin{prop}\label{ex30}
    ...
    \end{itemize}
\end{prop}
La demostración de esta última proposición es relativamente sencilla por inducción.