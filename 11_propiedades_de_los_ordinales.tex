...
A continuación se presenta la teoría de ordinales, su tratamiento en la teoría de conjuntos y sus relaciones con elementos importantes de la matemática, como los segmentos iniciales o los buenos órdenes.{\cite{SetTheory}}
\begin{defn}\label{d1} \textit{(ordinal)}\\
...
y todo subconjunto de $\alpha$ tiene primer elemento. Se podría señalar que la irreflexividad se sigue del axioma de Fundación.
\begin{thm}\label{t1}
...
Por otro lado, para cualesquiera $a,b\in\alpha$, si se cumple la tricotomía, tenemos que:
\[a\in b\lor a=b\lor b\in a.\]
De aquí, se sigue que $x\not\in x$ (es decir, se cumple la irreflexividad) y es una relación total. Además, si suponemos que $\alpha$ es transitivo, tenemos que $\in$ define un orden lineal estricto en $\alpha$.
Sea $A\subeq \al$ no vacío, entonces si suponemos que $A$ no tiene elemento mínimo, para cada $a\in A$ existe un $b\in A$ tal que
\[b\in a.\]
Tomemos un $c_0\in A$. Como no es mínimo, tenemos que existe $c_1\in A$ tal que
\[c_1\in c_0.\]
Repitiendo, tenemos
\[c_2\in c_1\land c_3\in c_2\land \cdots,\]
por lo que obtenemos una cadena descendente infinita
\[c_0\ni c_1\ni c_2\ni\cdots.\]
Lo cuál contradice el axioma de fundación.
Luego, todo subconjunto no vacío de $\alpha$ tiene elemento mínimo y $\alpha$ es ordinal.
\end{proof}

Considerando $\langle X,\prec\rangle$ un buen orden estricto. Entonces, $\prec$ es una relación bien fundada estricta sobre $X$. Luego, existe exactamente un conjunto transitivo $\alpha$ y una única biyección $F:X\longrightarrow\alpha$ tal que
\[(\forall x,x'\in X)(x\prec x'\Longleftrightarrow F(x)\in F(x')).\]

\begin{defn} \label{Mostowskixd}
    Sea $\langle X,\prec\rangle$ un buen orden estricto. Al conjunto transitivo $\alpha$ y a la biyección $F:\alpha\longrightarrow X$ que satisfacen
    \begin{equation}\tag{M}\label{M}
    (\forall x,x'\in X)(x\prec x'\Longleftrightarrow F(x)\in F(x'))
    \end{equation}    
    los llamamos el colapso de Mostowski de $\langle X,\prec\rangle$ y la función colapsante de Mostowski, respectivamente.
\end{defn}
...
\end{thm}
\begin{proof}
Si $\alpha$ es un ordinal, entonces $\alpha$ es el colapso de Mostowski de $\langle\alpha,\in\rangle$, con la función colapsante dada por la identidad. Por otro lado, si $\alpha$ es el colapso de Mostowski de un buen orden estricto $\langle X,\prec\rangle$, entonces $\alpha$ es transitivo. Además, para cualesquiera dos elementos distintos $a$ y $b$ en $X$, el conjunto $\set{a,b}$ tiene primer elemento, por lo que
\[a\prec b\lor b\prec a.\]
Y como la función colapsante es un isomorfismo de orden entre $\langle X,\prec\rangle$ y $\langle\alpha,\in\rangle$, tenemos que para cualesquiera dos elementos de $\alpha$ se cumple la tricotomía.
\end{proof}
...
\end{prop}
\begin{proof}
Sea $\alpha$ es colapso de Mostowski de $\la X,\prec_X\ra$ y $F:X\longrightarrow \alpha$ su función colapsante. Entonces, se satisface el enunciado {\eqref{M}} de {\ref{Mostowskixd}}. Luego, sea $\varphi:X\longrightarrow Y$ el isomorfismo de orden parcial estricto entre $Y$ y $X$.
...
\begin{prop}\label{p2}
WOT es una clase función que satisface lo siguiente:
\begin{enumerate}
    \item Si $\la x,y\ra\in\text{WOT}$, entonces $x$ y $\la y,\in\ra$ son buenos ordenes estrictos isomorfos. 
    \item El dominio de WOT es la clase de todos los buenos órdenes estrictos.
    \item Para cualesquiera buenos órdenes estrictos $x$ y $y$, y para todo ordinal $z$, si $\la x,z\ra\in\text{WOT}$ y $x\cong y$, entonces $\la y,z\ra\in\text{WOT}$.
\end{enumerate}
\end{prop}
\begin{proof} Que sea clase función viene del hecho de que el colapso de Mostowski es único. Por otro lado:
    \begin{enumerate}
...
\begin{defn}\label{d3}
    ...
    \begin{itemize}
        ...
        \item Sea $\lar{X,\preceq}$ un buen orden, y sea 
            \[\prec=\set{(x,y)\in X\times X|x\preceq y\land x\neq y},\] 
...
\end{proof}
\begin{defn}
    Sea $X$ un conjunto no vacío. Sea $x\in X$ y $Y\subseteq X$. Asumimos que $W$ es una relación bien fundada en $X$. El conjunto $\set{z\in X|\lar{z,x}\in W\land z\neq x}$ es llamado segmento inicial de $\lar{X,W}$ determinado por $x$, y denotado por $I_W(x)$. Decimos que $x$ es $W\text{-minimal}$ para $Y$ si 
...
\begin{prop}
    Si $\lar{X,\preceq}$ es un buen orden y\hspace{0.1cm} $Y$ es un segmento inicial de $X$, entonces 
...
\end{cor}
\begin{proof}
    Por simplicidad, escribiremos $\in$ en lugar de $\in\cap\ON\times\ON$. Procederemos explicando dos casos:
    \begin{itemize}
        \item Cualquier conjunto (no vacío) de $\ON$ tiene elemento mínimo: Sea $X$ un conjunto (no vacío) cuyos elementos son ordinales. Denotamos:
            \[\alpha:=\bigcap X.\] 
        ...
        \item Cualquier clase (no vacía) de $\ON$ tiene elemento mínimo: Consideremos la clase (no vacía) $\mathcal{X}$ definida por la fórmula (de teoría de conjuntos) $\varphi(x)$ y cuyos elementos son todos ordinales. Tomemos $\alpha$ tal que $\varphi(\alpha)$ (es decir, $\alpha$ pertenece a la clase $\mathcal{X}$). Tenemos dos casos: 
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Si $\alpha\neq\beta$, y tomamos $\gamma\in\beta\setminus\alpha$; como $\beta\not\in\alpha$, tenemos que se cumple $\alpha=\gamma$ o $\alpha\in\gamma\in\beta$, pero no ambas.
...
La propiedad anterior muestra que, para cualquier ordinal $\alpha$, su sucesor $S(\alpha)$ también es un ordinal. Más aún, como $S(\alpha)=\alpha\cup\set\alpha$, tenemos que $\alpha\in S(\alpha)$, y si $\beta\in S(\alpha)$, entonces $\beta\in\alpha$ o $\beta=\alpha$, pero no ambos. Por lo tanto, $S(\alpha)$ es el más pequeño ordinal, mayor que $\alpha$; por lo que $S(\alpha)$ es el sucesor inmediato de $\alpha$ en $\ON$.\\
El conjunto vacío es un ordinal. Esto se ve de que $\varnothing$ satisface la definición, o porque $\varnothing$ es el colapso de Mostowski de $\lar{\varnothing,\varnothing}$. Más aún, como $\varnothing$ no contiene elementos, este debería ser el ordinal más pequeño. Los siguientes ordinales son $S(\nt),S(S(\nt)),\ldots$, etcétera. En la teoría de conjuntos, se denota al $n-$ésimo sucesor de $\nt$ por $\widetilde n$ y se identifica por el número natural $n$. Entonces, ahora vemos que cada $\widetilde n$ es un ordinal; más aún, la relación $<$ restringida a los ordinales $\widetilde n$ es el orden estricto usual de los números naturales, y $\widetilde n=\ot(\lar{\set{0,1,\ldots,n-1},<})$. Así, los números naturales son los ordinales finitos.
\begin{prop}
    Un ordinal $\alpha$ es un conjunto finito si, y sólo si es un número natural.
\end{prop}
...
\begin{proof}
    ...
    Como $\alpha\in\omega$, entonces $\alpha$ se identifica por un número natural $n$. Por como se definió $\alpha$ y como $n-1\in n=\alpha$, entonces $S(n-1)\in\alpha=n$, luego $n\in n$, lo cual es absurdo.
\end{proof}
\begin{thm}\label{t10}
...
\end{thm}
Los siguientes resultados muestran la relación entre los ordinales y los buenos órdenes, entre otros conceptos asociados.
\begin{defn}
    ...
    \[(\forall z\in Z)(\rk(z)={\sup}\set[S(\rk(y))]{y W z}.\]
\end{defn}
\begin{prop}
    \text{\  }
    \begin{enumerate}
        \item Sea $W$ una relación bien fundada sobre un conjunto $Z$, $\lar{X,\prec}$ un buen orden estricto, y $\rk:Z\longrightarrow X$ una función rango para $W$ con respecto a $\prec$. Sea $\alpha=\ot(\lar{X,\prec})$, y sea $F:X\longrightarrow\alpha$ el isomorfismo de orden entre $\lar{X,\prec}$ y $\lar{\alpha,\in}$. Entonces la composición $F\circ\rk:Z\longrightarrow\alpha$ es una función rango para $W$ con respecto a $\in$.
        \item Para cada relación bien fundada, existe una única función rango con respecto a $\in$ cuya imagen es un ordinal.
    \end{enumerate}
\end{prop}
\begin{proof}\hspace{0.45cm}
    \begin{enumerate}
        \item Dado que los conjuntos están bien ordenados y que $F$ es un isomorfismo de orden, tenemos que:
            \[S(F(x))=F(S(x)).\]
            Luego,
            \[\set[S(F(\rk(y)))]{y W z}=F{\set[S(\rk(y))]{yWz}}.\]
            Así, el supremo de ambos conjuntos coincide y es $F(rk(z))$ para cada $z$.
        \item La existencia viene asegurada por el axioma de elección. Supongamos que existen dos funciones diferentes $f$ y $g$ tales que ambas son funciones rango. Sin pérdida de generalidad, supongamos que el contradominio de ambas es $f(Z)$ y $g(Z)$.
            Consideremos que la regla de asignación es diferente. Sin pérdida de generalidad, supongamos $f(Z)\subsetneqq g(Z)$. Por el axioma de fundación, $g(Z)$ está bien ordenado, por lo que bajo $g^{-1}$ podemos bien ordenar $Z$. Luego, $f$ debe seguir el mismo orden impuesto en $Z$ por $g^{-1}$ (por la definición de función rango), así los supremos de los conjuntos
            \[\set[S(f(y))]{y W z},\]
            y
            \[\set[S(g(y))]{y W z}\] 
            deben de coincidir, lo cuál contradice que las reglas de asignación sean diferentes, luego la función rango es única.
    \end{enumerate}
\end{proof}
\begin{defn}
    $R$ es una relación bien fundada en $X$ si es una relación y para cada $Y\subeq X$, no vacío,
...
\end{proof}
\begin{thm}\label{t14}
    (ZF): Para cada conjunto $x$, existe un ordinal $\alpha>0$ tal que $x$ no puede mapearse de manera sobreyectiva en $\alpha$.
\end{thm}
\begin{proof}
    Consideremos 
    \[\mathcal{W}=\set[R\subeq x\times x]{R\text{ es estrictamente bien fundada sobre }x}.\]
    Ahora, para cada $R$ en $\mathcal W$, definimos
    \[\rk_R=x\longrightarrow\ON\]
    la función rango asociada a la relación $R$. Consideremos entonces:
    \[A=\set[\rk_R(x)]{R\in\mathcal{W}}.\]
    Tenemos que $A$ es un conjunto de ordinales. Así, podemos definir:
    \[\beta=\bigcup\bigcup A,\]
    el cuál es un ordinal. Tomemos entonces $\alpha=\beta+1$. Si suponemos que existe una función sobreyectiva $f:x\longrightarrow \alpha$, entonces, por {\ref{p15}} existe una relación bien fundada $R_f$ tal que 
    \[f=\rk_{R_f}.\]
    Como $f$ es sobreyectiva sobre $\alpha$, tenemos que
    \[f(x)=\alpha.\]
    Así, $\alpha=\rk_{R_f}(x)\in A$.
    Pero, por como definimos $\beta$, si $\gamma\in A$, entonces
    \[\gamma\subeq \beta,\]
    luego $\gamma\leq\beta<\beta+1=\alpha$. Por lo tanto, ningún elemento de $A$ puede coincidir con $\alpha$, es decir
    \[\alpha\not\in A,\]
    lo cual es una contradicción.
    Así, tenemos lo que queríamos demostrar.
\end{proof}
\begin{prop}\label{ex17}
    Existe una fórmula $\Psi(r,\alpha)$ de $L_s$ con dos variables libres tal que $\Psi(r,\alpha)$ es válida si, y sólo si $r$ es una relación de buen orden estricto en $x$ y $\alpha$ es el rango de la función rango para $r$.
\end{prop}
\begin{cor}\label{c16}
...
\end{proof}
\begin{lem}\label{l17}
    (ZF) Si cada conjunto no vacío admite estructura de grupo, entonces cada conjunto puede ser bien ordenado.
\end{lem}
\begin{proof}
    ...
    En efecto, fijamos $z\in x$ y consideremos la función $f:\alpha\longrightarrow y$ dada por la fórmula $f(\beta)=z*\beta$. Como $z^{-1}*f(\beta)=\beta$, para cada $\beta\in\alpha$, la función $f$ es intectiva. Por la elección de $\alpha$, el rango de $f$ no está contenido en $x$. Pero, si $f(\beta)\in y\setminus x$, entonces $f(\beta)\in\alpha$, lo que prueba la proposición.
    Ahora, sea $\preceq_{a}$ el orden antilexicográfica sobre $\alpha\times\alpha$. Ahora, claro que $\preceq_a$ es una relación de buen orden. Definimos una función $g:x\longrightarrow\alpha\times\alpha$ siendo $g(z)$ el $\preceq_a$-elemento mínimal $\lar{\beta,\gamma}$ de $\alpha\times\alpha$ tal que $z*\beta=\gamma$. Notemos que si $g(z)=\lar{\beta,\gamma}$, entonces $z=\gamma*\beta^{-1}$. Así, $g$ es una función inyectiva. Luego, definimos la relación $\preceq$ sobre $x$ por: 
...
\begin{thm}
    ...
    Sea $X$ un conjunto no vacío, sea $Y\subeq X$ y sea $W$ una relación bien fundada sobre $X$. Si la implicación
...
\end{thm}
\begin{proof}
    Sea $X,Y$ y $W$ como en el enunciado y asumimos que {\eqref{Ind}} es válida para cada $x\in X$. Supongamos $C=X\setminus Y\neq \nt$. Por la bien fundación de $W$, existe un elemento $W-$mínimal de $C$. Pero, si $x_o$ es un elemento $W-$mínimal de $C$, entonces $I_W(x_o)\subeq Y$, y de {\eqref{Ind}} (considerada con $x_o$) se sigue que $x_o\in Y$, lo cuál es imposible, pues $x_o\in X\setminus Y$, lo cuál es una contradicción.
\end{proof}
\begin{thm}
    (Principio de Definición Recursiva Generalizado)\\
    Sean $X,Z\neq\nt$, y sea $W$ una relación bien fundada sobre $Z$. Además, supónganse que $G$ es una función con valores en $X$ tal que $\text{dom}(G)$ consiste de todos los pares de la forma $\lar{f,z}$, donde $z\in Z$ y $f$ es una función que mapea $I_W(z)$ en $X$. Entonces, existe exactamente una función $F:Z\longrightarrow X$ tal que
...
\begin{thm}
    ...
    Sean $X,Z\neq\nt$, y sea $W$ una relación bien fundada sobre $Z$. Además, supónganse que $G^*$ es una función con valores en $\PX\setminus\set{\nt}$ tal que $\text{dom}(G^*)$ consiste de todos los pares de la forma $\lar{f,z}$, donde $z\in Z$ y $f$ es una función que mapea $I_W(z)$ en $X$. Entonces, existe una función $F:Z\longrightarrow X$ tal que
    \begin{equation}\tag{R*}\label{R*}
        (\forall z\in Z)(F(z)\in G^*(F|I_W(z),z))
    \end{equation}
\end{thm}
Consideremos un ordinal arbitrario $\alpha$. Como $\in$ es una relación bien fundada sobre $\alpha$, son válidos los teoremas anteriores. La inducción y recursión sobre $\in$ en ordinales son comúnmente referenciadas como \textbf{inducción transfinita} y \textbf{recursión transfinita}.
\begin{thm}\label{t19}
    (ZF) Sea $X$ un conjunto, sea $C$ una familia no vacía de subconjuntos de $X$ que es cerrado bajo uniones de subfamilias que son linealmente ordenados por inclusión, y sea $f:C\longrightarrow C$ una función con 
