...
\begin{defn} {\label{definiblexd}}
    Un conjunto $X\subseteq\n^k$ es definible si existe una fórmula $\varphi(x_1,\ldots,x_k)$ tal que
    \[X=\{(a_1,\ldots,a_k)|\n\vDash\varphi[a_1,\ldots,a_k]\}.\]
\end{defn}

\begin{obs}
    Señalaremos una nueva forma de codificar parejas:
    \[(a,b)\text{ se codifica por }(a+b)^2+a.\]
    Dado $n$, me fijo en la sucesión de cuadrados perfectos ${(n^2)}_{n\in\n}$, y encuentro $m$ tal que $m^2\leq n<{(m+1)}^2$. Hacemos $a=n-m^2$, $b=m-a$, y $n$ codifica $(a,b)$. Notemos:
...
\end{obs}
De la observación anterior la fórmula que significa ``$z$ codifica a $(x,y)$'', para futuras referencias, la llamaremos Par$(x,y,z)$.
\begin{lem}
...
\end{obs}
De la anterior observación la fórmula que significa ``$x$ es la $y-$ésima entrada de la tupla codificada por $z$'', para futuras referencias, la llamaremos Tupla$(x,y,z)$. Asimismo, a la fórmula que  significa ``la longitud de la tupla codificada por $z$ es $y$'' la llamaremos Long$(y,z)$.
\begin{obs}
    $T$ máquina de Turing.
    ...
    Así pues,
    \begin{itemize}
        \item ``$(x,y,z)$ es una foto instantánea válida'', que llamaremos FI$(x,y,z)$, viene dada por:\\
        ...
        \item ``$(x_1,y_1,z_1)$ es la foto instantánea inmediatamente posterior a $(x_2,y_2,z_2)$'', que llamaremos T$(x_1,y_1,z_1,x_2,y_2,z_2)$, viene dada por:\\
            $\left(''(x_{1}, y_{1}, z_{1}) \, \text{es foto inst. válida}'' \right)
\;\land\;
...
$.
        \item ``si yo corro $T$ con input $x$, entonces la máquina se detiene y el output es $y$'', que llamaremos C$(x,y)$, viene dada por:\\
...
\end{thm}
\begin{proof}
    Supongamos que sí es decidible, es decir, existe una enumeración efectiva de las fórmulas $\varphi_1,\ldots,\varphi_n,\ldots$ y un algoritmo que calcula correctamente 
...
\end{cor}
\begin{proof}
    De lo contrario, si $\Sigma\subseteq \text{Th}(\cN)$ es computable y completo, entonces:
...
\begin{defn} \textit{(Aritmética de Robinson)}\\
    El conjunto $Q$ (RA) consta de las siguientes fórmulas:
    \begin{enumerate}
...
\begin{proof}\hspace{0.5cm}
    \begin{enumerate}
        \item Probemos que Teor$(\Sigma\cup Q)$ no es computable. Sea $\varphi_1(x),\varphi_2(x),\ldots,\varphi_n(x),\ldots$ una enumeración efectiva de las fórmulas con una variable libre. Sea
            ...
            Sea $e$ tal que $\neg\psi$ es la fórmula $\varphi_e$. Entonces,
            \begin{align*}
