...
También conocido por su nombre en alemán \textit{Entscheindungsproblem}, o en inglés \textit{halting problem}, este concepto es de importancia capital para entender las limitaciones de la computabilidad, lo cuál eventualmente nos llevará a la incompletitud. \cite{comp1}
...
\begin{proof}
    \begin{enumerate}
        \item La demostración se sigue del siguiente pseudocódigo:
            \begin{verbatim}
                input(m)
                for(k=0;k<n;k++)
                    if(Xx(m,k))
...
\end{proof}
\begin{obs}
    El recíproco en general es falso, y el contraejemplo viene en los últimos ejemplos dados.
\end{obs}
\begin{prop}
...
\begin{thm}
    ...
    \begin{enumerate}
        \item $X$ es c.e.
        \item La función semicaracterística de $X$, denotada $\sigma_X$, es computable. Esta función viene dada por la regla de asignación:
